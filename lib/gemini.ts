...
export async function callGemini(prompt: string): Promise<string> {
  ...
  try {
    ...
    if (!parser.has("candidates") || !parser.isArray("candidates")) {
      console.error("Invalid Gemini Response: Missing 'candidates' array", responseData);
      throw new Error("Invalid response format from Gemini API: Missing candidates.");
    }

    const candidates = parser.getArray("candidates");
    if (candidates.length === 0) {
      console.error("Invalid Gemini Response: Empty 'candidates' array", responseData);
    ...
    if (firstCandidateParser.has("finishReason") && firstCandidateParser.getString("finishReason") !== "STOP") {
      console.warn(`Gemini response finished with reason: ${firstCandidateParser.getString("finishReason")}`);
      // Decide how to handle non-STOP finishes (e.g., throw error, return partial, etc.)
    }


    if (!firstCandidateParser.has("content", "parts") || !firstCandidateParser.isArray("content", "parts")) {
      console.error("Invalid Gemini Response: Missing 'content.parts' array", candidates[0]);
      throw new Error("Invalid response format from Gemini API: Missing content parts.");
    }

    const parts = firstCandidateParser.getArray("content", "parts");
    if (parts.length === 0 || typeof parts[0] !== 'object' || parts[0] === null || !('text' in parts[0]) || typeof parts[0].text !== 'string') {
      console.error("Invalid Gemini Response: Missing 'text' in first part", parts);
      throw new Error("Invalid response format from Gemini API: Missing text in response part.");
    }
